...
void importDataset(std::vector<std::vector<INPUT_DATA_TYPE>> *inputVector,
  ...
  const unsigned int totalPoints =
      (unsigned int)tmpAllData.size() / INPUT_DATA_DIM;
  fprintf(stdout, "[Dataset] ~ Data import: Total data points: %d\n",
          totalPoints);
  fprintf(stdout,
          "[Dataset] ~ Data import: Total size of all data (1-D) vect (number "
          "of points * GPUNUMDIM): %zu\n",
          tmpAllData.size());
...
void importDataset_v2(std::vector<std::vector<INPUT_DATA_TYPE>> *inputVector,
  ...
  while (fscanf(fptr, "%lf, ", &check) == 1 ||
         fscanf(fptr, "%lf ", &check) == 1) {
