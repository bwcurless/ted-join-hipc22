...
GPUBatchEst(unsigned int *nbQueryPoints, INPUT_DATA_TYPE *dev_database,
            unsigned int *dev_originPointIndex, ACCUM_TYPE *dev_epsilon,
            struct grid *dev_grid, unsigned int *dev_gridLookupArr,
            struct gridCellLookup *dev_gridCellLookupArr,
            INPUT_DATA_TYPE *dev_minArr, unsigned int *dev_nCells,
            unsigned int *dev_nNonEmptyCells, unsigned int *retNumBatches,
  ...
  cudaErrCheck(cudaMemcpy(dev_N_batchEst, N_batchEst, sizeof(unsigned int),
                          cudaMemcpyHostToDevice));

  unsigned int *dev_cnt_batchEst;
  cudaErrCheck(cudaMalloc((void **)&dev_cnt_batchEst, sizeof(unsigned int)));
  cudaErrCheck(cudaMemcpy(dev_cnt_batchEst, cnt_batchEst, sizeof(unsigned int),
                          cudaMemcpyHostToDevice));

  unsigned int *dev_sampleOffset;
  cudaErrCheck(cudaMalloc((void **)&dev_sampleOffset, sizeof(unsigned int)));
  cudaErrCheck(cudaMemcpy(dev_sampleOffset, &offsetRate, sizeof(unsigned int),
                          cudaMemcpyHostToDevice));

  unsigned int *dev_estimatedResult;
  cudaErrCheck(cudaMalloc((void **)&dev_estimatedResult,
                          (*N_batchEst) * sizeof(unsigned int)));

  unsigned int *dev_nbCandidatesEst;
  cudaErrCheck(cudaMalloc((void **)&dev_nbCandidatesEst,
                          (*N_batchEst) * sizeof(unsigned int)));

  unsigned int nbBlock = ceil((1.0 * (*N_batchEst)) / (1.0 * BLOCKSIZE));

  batchEstimatorKernel<<<nbBlock, BLOCKSIZE>>>(
      dev_N_batchEst, dev_sampleOffset, dev_database, dev_originPointIndex,
      dev_epsilon, dev_grid, dev_gridLookupArr, dev_gridCellLookupArr,
      dev_minArr, dev_nCells, dev_cnt_batchEst, dev_nNonEmptyCells,
      dev_estimatedResult, dev_nbCandidatesEst);

  cudaErrCheck(cudaMemcpy(cnt_batchEst, dev_cnt_batchEst, sizeof(unsigned int),
                          cudaMemcpyDeviceToHost));
  cudaErrCheck(cudaMemcpy(estimatedResult, dev_estimatedResult,
                          (*N_batchEst) * sizeof(unsigned int),
                          cudaMemcpyDeviceToHost));
  cudaErrCheck(cudaMemcpy(estimateCandidates, dev_nbCandidatesEst,
                          (*N_batchEst) * sizeof(unsigned int),
                          cudaMemcpyDeviceToHost));

  fprintf(stdout,
          "[GPU Est] ~ Result set size for estimating the number of batches "
          "(sampled): %d\n",
          (*cnt_batchEst));
  ...
  for (unsigned int i = 0; i < nbUnestimatedSequences - 1; ++i) {
    ...
    unsigned int maxCandidates = (nbCandidatesBefore < nbCandidatesAfter)
                                     ? nbCandidatesAfter
                                     : nbCandidatesBefore;
  ...
  std::cout << "[GPU Est] ~ Estimated total result set size: " << fullEst
            << '\n';
  std::cout << "[GPU Est] ~ Estimated number of candidate points: "
            << fullEstCandidates << '\n';
  // fprintf(stdout, "[GPU Est] ~ Estimated total result set size: %lu\n",
  // fullEst); fprintf(stdout, "[GPU Est] ~ Estimated number of candidate
  // points: %lu\n", fullEstCandidates);

  if (fullEst < (GPUBufferSize * GPUSTREAMS)) {
    GPUBufferSize =
        min((int)(ceil((1.0 * fullEst.convert_to<int>()) / (1.0 * GPUSTREAMS)) *
                  1.10),
            GPU_BUFFER_SIZE);
    // GPUBufferSize = ceil((1.0 * fullEst.convert_to<int>()) / (1.0 *
    // GPUSTREAMS)); GPUBufferSize = (fullEst.convert_to<int>() / GPUSTREAMS) +
    // 1;
    fprintf(
        stdout,
        "[GPU Est] ~ Result set size too small, reducing GPUBufferSize to %d\n",
        GPUBufferSize);
  }
  ...
  // Keeping a 5% margin to avoid a potential buffer overflow due to a wrong
  // estimation of the result set size
  ...
  for (unsigned int i = 0; i < (*nbQueryPoints); ++i) {
    ...
    if ((GPUBufferSize - reserveBuffer) <= runningEst) {
      ...
      // fprintf(stdout, "[Batch Est] ~ New batch: begin = %u, end = %u,
      // candidates = %llu, neighbors = %llu\n",
      //     newBatch.begin, newBatch.end, newBatch.nbCandidates,
      //     newBatch.nbNeighborsEst);
      batches->push_back(newBatch);
      batchBegin = i;
      runningEst = 0;
      runningCandidates = 0;
    } else {
      // The last batch is unlikely to pass the above condition, so we force its
      // creation when the end of the dataset is reached
      if (i == ((*nbQueryPoints) - 1)) {
        struct batch newBatch = {batchBegin, (*nbQueryPoints),
                                 runningCandidates, runningEst};
        batches->push_back(newBatch);
        runningEst = 0;
        runningCandidates = 0;
      }
...
void GPUJoinMainIndex(
    int searchMode, INPUT_DATA_TYPE *dataset, INPUT_DATA_TYPE *dev_database,
    unsigned int *nbQueryPoints, ACCUM_TYPE *epsilon, ACCUM_TYPE *dev_epsilon,
    struct grid *grid, struct grid *dev_grid, unsigned int *gridLookupArr,
    unsigned int *dev_gridLookupArr, struct gridCellLookup *gridCellLookupArr,
    struct gridCellLookup *dev_gridCellLookupArr, INPUT_DATA_TYPE *minArr,
    INPUT_DATA_TYPE *dev_minArr, unsigned int *nCells, unsigned int *dev_nCells,
    unsigned int *nNonEmptyCells, unsigned int *dev_nNonEmptyCells,
    unsigned int *originPointIndex, unsigned int *dev_originPointIndex,
    struct neighborTableLookup *neighborTable,
    std::vector<struct neighborDataPtrs> *pointersToNeighbors,
    uint64_t *totalNeighbors, uint64_t *totalNeighborsCuda,
    ...
    unsigned int *totalQueriesCuda, unsigned int *totalQueriesTensor,
    unsigned int *totalKernelsCuda, unsigned int *totalKernelsTensor,
  ...
  cudaErrCheck(cudaMalloc(
      (void **)&dev_datasetAlt,
      sizeof(half) * ((*nbQueryPoints) + ADDITIONAL_POINTS) * COMPUTE_DIM));
  unsigned int nbBlock =
      ceil((1.0 * ((*nbQueryPoints) + ADDITIONAL_POINTS)) / (1.0 * BLOCKSIZE));
  convertDataset<<<nbBlock, BLOCKSIZE>>>(
      dev_database, dev_datasetAlt, ((*nbQueryPoints) + ADDITIONAL_POINTS));
  ...
  if (searchMode == SM_TENSOR_MQ_HYBRID) {
    unsigned int nbBlocks = ceil(
        (1.0 * ((*nbQueryPoints) + ADDITIONAL_POINTS)) / (1.0 * BLOCKSIZE));
    // Pre-compute the square of points' coordinates
    cudaErrCheck(
        cudaMalloc((void **)&dev_preComputedSquaredCoordinates,
                   sizeof(double) * ((*nbQueryPoints) + ADDITIONAL_POINTS) *
                       (COMPUTE_DIM / 4)));
    preComputedSquaredCoordinates<<<nbBlocks, BLOCKSIZE>>>(
        dev_database, dev_preComputedSquaredCoordinates,
  ...
  *totalResultSetCnt = 0;
  ...
  cudaErrCheck(
      cudaMalloc((void **)&dev_cnt, sizeof(unsigned int) * GPUSTREAMS));
  ...
  cudaErrCheck(
      cudaMalloc((void **)&dev_offset, sizeof(unsigned int) * GPUSTREAMS));

  unsigned int *batchNumber = new unsigned int[GPUSTREAMS];
  unsigned int *dev_batchNumber;
  cudaErrCheck(
      cudaMalloc((void **)&dev_batchNumber, sizeof(unsigned int) * GPUSTREAMS));
  ...
  estimatedNeighbors = GPUBatchEst(
      nbQueryPoints, dev_database, dev_originPointIndex, dev_epsilon, dev_grid,
      dev_gridLookupArr, dev_gridCellLookupArr, dev_minArr, dev_nCells,
      dev_nNonEmptyCells, &numBatches, &GPUBufferSize, &batchesVector);
  ...
  cout << "[GPU] ~ In calling function: Estimated neighbors = "
       << estimatedNeighbors << ", num. batches = " << numBatches
       << ", GPU buffer size = " << GPUBufferSize << '\n';
  cout.flush();

#if VERBOSITY >= 2
  fprintf(stdout, "[GPU] ~ Regular batches: \n");
  for (unsigned int i = 0; i < batchesVector.size(); ++i) {
    fprintf(stdout, "  Batch %u: %u, %u (%u)\n", i, batchesVector[i].begin,
            batchesVector[i].end,
  ...
  runningPointsBatch =
      batchesVector[batchIndex].end - batchesVector[batchIndex].begin;

  while (pointIndex < (*nbQueryPoints)) {
    if ((pointIndex + NB_QUERY_POINTS_TENSOR) < runningPointsCell &&
        (pointIndex + NB_QUERY_POINTS_TENSOR) < runningPointsBatch) {
      ...
      if ((pointIndex + NB_QUERY_POINTS_TENSOR) < runningPointsCell &&
        ...
        workQueueTensorBatches.push_back(pointIndex);
        runningTensorBatchEnd++;
        tensorBatchesIndices.push_back(
            std::make_pair(runningTensorBatchBegin, runningTensorBatchEnd));
        ...
        if ((pointIndex + NB_QUERY_POINTS_TENSOR) >= runningPointsCell &&
            (pointIndex + NB_QUERY_POINTS_TENSOR) < runningPointsBatch) {
          // The tensor batch is within the regular batch but exceeds the
          // current cell
          ...
          cellId = sortedCells[cellIndex].cellId;
          runningPointsCell +=
              grid[cellId].indexmax - grid[cellId].indexmin + 1;
        } else {
          if ((pointIndex + NB_QUERY_POINTS_TENSOR) >= runningPointsCell &&
              (pointIndex + NB_QUERY_POINTS_TENSOR) >= runningPointsBatch) {
            ...
            if (runningPointsCell < runningPointsBatch) {
              ...
              if (runningPointsBatch < runningPointsCell) {
                ...
                workQueueTensorBatches.push_back(pointIndex);
                runningTensorBatchEnd++;
                tensorBatchesIndices.push_back(std::make_pair(
                    runningTensorBatchBegin, runningTensorBatchEnd));
                runningTensorBatchBegin = runningTensorBatchEnd;
                batchIndex++;
                runningPointsBatch += batchesVector[batchIndex].end -
                                      batchesVector[batchIndex].begin;
              } else {
                // Both the regular batch and the cell end at the same point
                // Unlikely, and should most often correspond to the end of the
                // dataset
                pointIndex = runningPointsCell;
                workQueueTensorBatches.push_back(pointIndex);
                runningTensorBatchEnd++;
                ...
                if (pointIndex < (*nbQueryPoints)) {
                  // If there are query points left, then having the regular
                  // batch and the cell ending at the same point is just chance,
                  // and so other batches will be created. But if there are no
                  // more query points left, then we do not update the variables
                  // below.
                  runningTensorBatchBegin = runningTensorBatchEnd;
                  cellIndex++;
                  cellId = sortedCells[cellIndex].cellId;
  ...
  for (unsigned int i = 0; i < tensorBatchesIndices.size(); ++i) {
    fprintf(stdout, "  Tensor batch %u: %u, %u (%u)\n", i,
            tensorBatchesIndices[i].first, tensorBatchesIndices[i].second,
  ...
  cudaErrCheck(cudaMalloc((void **)&dev_tensorBatches,
                          nbTensorBatches * sizeof(unsigned int)));
  cudaErrCheck(cudaMemcpy(dev_tensorBatches, tensorBatches,
                          nbTensorBatches * sizeof(unsigned int),
                          cudaMemcpyHostToDevice));

  unsigned int *dev_tensorBatchesSize;
  cudaErrCheck(
      cudaMalloc((void **)&dev_tensorBatchesSize, sizeof(unsigned int)));
  cudaErrCheck(cudaMemcpy(dev_tensorBatchesSize, &nbTensorBatches,
  ...
  for (int i = 0; i < GPUSTREAMS; i++) {
    cudaErrCheck(cudaMalloc((void **)&dev_pointIDKey[i],
                            2 * sizeof(int) * GPUBufferSize));
    cudaErrCheck(cudaMalloc((void **)&dev_pointInDistValue[i],
                            2 * sizeof(int) * GPUBufferSize));
  }
  fprintf(stdout,
          "[GPU] ~ Allocation pointIDKey and pointInDistValue on the GPU, size "
          "= %lu\n",
          2 * sizeof(int) * GPUBufferSize);
...
#pragma omp parallel for num_threads(GPUSTREAMS)
  for (int i = 0; i < GPUSTREAMS; i++) {
    ...
    cudaMallocHost((void **)&pointInDistValue[i],
                   2 * sizeof(int) * GPUBufferSize);
  }
  ...
  fprintf(stdout, "[GPU] ~ Memory request for results on GPU: %f GiB\n",
          (double)(sizeof(int) * 2 * GPUBufferSize * GPUSTREAMS) /
              (1024 * 1024 * 1024));
  fprintf(stdout,
          "[GPU] ~ Memory requested for results in main memory: %f GiB\n",
          (double)(sizeof(int) * 2 * GPUBufferSize * GPUSTREAMS) /
              (1024 * 1024 * 1024));
  ...
  // unsigned int batchesThatHaveOneMore = (*nbQueryPoints) - (batchSize *
  // numBatches); //batch number 0- < this value have one more
  unsigned int batchesThatHaveOneMore = datasetSize - (batchSize * numBatches);
  fprintf(stdout,
          "[GPU] ~ Batches that have one more GPU thread: %d, batchSize: %d\n",
  ...
  cudaErrCheck(
      cudaMalloc((void **)&dev_batchBegin, GPUSTREAMS * sizeof(unsigned int)));

  unsigned int *dev_batchEnd;
  cudaErrCheck(
      cudaMalloc((void **)&dev_batchEnd, GPUSTREAMS * sizeof(unsigned int)));
  ...
  boost::multiprecision::uint128_t *nbCandidatesCuda =
      new boost::multiprecision::uint128_t[GPUSTREAMS];
  boost::multiprecision::uint128_t *nbCandidatesTensor =
      new boost::multiprecision::uint128_t[GPUSTREAMS];
...
#pragma omp parallel for schedule(dynamic, 1) reduction(+ : totalResultsLoop)  \
    num_threads(GPUSTREAMS)
  for (int i = 0; i < numBatches; ++i) {
    ...
    case SM_GPU:
      // case SM_GPU_HALF2:
      {
        cudaErrCheck(cudaMemcpy(&dev_batchBegin[tid], &batchesVector[i].begin,
                                sizeof(unsigned int), cudaMemcpyHostToDevice));
        cudaErrCheck(cudaMemcpy(&dev_batchEnd[tid], &batchesVector[i].end,
                                sizeof(unsigned int), cudaMemcpyHostToDevice));
    ...
    case SM_TENSOR_MQ_HYBRID: {
      cudaErrCheck(cudaMemcpy(&dev_batchBegin[tid],
                              &tensorBatchesIndices[i].first,
                              sizeof(unsigned int), cudaMemcpyHostToDevice));
      cudaErrCheck(cudaMemcpy(&dev_batchEnd[tid],
                              &tensorBatchesIndices[i].second,
                              sizeof(unsigned int), cudaMemcpyHostToDevice));
      N[tid] =
          (tensorBatchesIndices[i].second - tensorBatchesIndices[i].first) *
          WARP_SIZE;
      ...
      // nbBlock = ceil(((1.0 * N[tid]) / (1.0 * tensorBlockSize)) * (WARP_SIZE
      // / NB_QUERY_POINTS_TENSOR));
...
#if VERBOSITY >= 2
    cout << "[GPU] ~ Batch " << i << ": N = " << N[tid]
         << ", nbBlock = " << nbBlock << endl;
    // cout << "[GPU] ~ N (1 less): " << N[tid] << ", tid " << tid << '\n';
#endif

    cudaErrCheck(cudaMemcpyAsync(&dev_N[tid], &N[tid], sizeof(unsigned int),
                                 cudaMemcpyHostToDevice, stream[tid]));

    cnt[tid] = 0;
    cudaErrCheck(cudaMemcpyAsync(&dev_cnt[tid], &cnt[tid], sizeof(unsigned int),
                                 cudaMemcpyHostToDevice, stream[tid]));

    batchOffset[tid] = numBatches; // for the strided
    cudaErrCheck(cudaMemcpyAsync(&dev_offset[tid], &batchOffset[tid],
                                 sizeof(unsigned int), cudaMemcpyHostToDevice,
                                 stream[tid]));

    batchNumber[tid] = i;
    cudaErrCheck(cudaMemcpyAsync(&dev_batchNumber[tid], &batchNumber[tid],
                                 sizeof(unsigned int), cudaMemcpyHostToDevice,
                                 stream[tid]));
    ...
    case SM_GPU: {
      ...
      distanceCalculationGridCuda<<<nbBlock, BLOCKSIZE, 0, stream[tid]>>>(
          &dev_batchBegin[tid], &dev_N[tid], dev_datasetAlt,
          dev_originPointIndex, dev_epsilon, dev_grid, dev_gridLookupArr,
          dev_gridCellLookupArr, dev_minArrAlt, dev_nCells, &dev_cnt[tid],
          dev_nNonEmptyCells, dev_pointIDKey[tid], dev_pointInDistValue[tid]);
#else
      distanceCalculationGridCuda<<<nbBlock, BLOCKSIZE, 0, stream[tid]>>>(
          &dev_batchBegin[tid], &dev_N[tid], dev_database, dev_originPointIndex,
          dev_epsilon, dev_grid, dev_gridLookupArr, dev_gridCellLookupArr,
          dev_minArr, dev_nCells, &dev_cnt[tid], dev_nNonEmptyCells,
    ...
    case SM_TENSOR_MQ_HYBRID: {
      ...
      distanceCalculationGridTensor_multiQueryPoints_double_8_8_4_tensor_mixed<<<
          ...
          dev_grid, dev_gridLookupArr, dev_gridCellLookupArr, dev_minArr,
          dev_nCells, &dev_cnt[tid], dev_nNonEmptyCells, dev_pointIDKey[tid],
          dev_pointInDistValue[tid]);
#endif
      break;
    }
    }
    ...
    errCode = cudaMemcpyAsync(&cnt[tid], &dev_cnt[tid], sizeof(unsigned int),
                              cudaMemcpyDeviceToHost, stream[tid]);
    if (errCode != cudaSuccess) {
      cout << "[GPU] ~ Error: getting cnt from GPU Got error with code "
           << errCode << '\n';
      cout << "  Details: " << cudaGetErrorString(errCode) << endl;
    } else {
    ...
    try {
      thrust::sort_by_key(thrust::cuda::par.on(stream[tid]), dev_keys_ptr,
                          dev_keys_ptr + cnt[tid], dev_data_ptr);
    ...
    cudaMemcpyAsync(thrust::raw_pointer_cast(pointIDKey[tid]),
                    thrust::raw_pointer_cast(dev_keys_ptr),
                    cnt[tid] * sizeof(int), cudaMemcpyDeviceToHost,
                    stream[tid]);
    cudaMemcpyAsync(thrust::raw_pointer_cast(pointInDistValue[tid]),
                    thrust::raw_pointer_cast(dev_data_ptr),
                    cnt[tid] * sizeof(int), cudaMemcpyDeviceToHost,
                    stream[tid]);
    ...
    constructNeighborTableKeyValueWithPtrs(
        pointIDKey[tid], pointInDistValue[tid], neighborTable,
        (*pointersToNeighbors)[i].dataPtr, &cnt[tid]);

    double tableconstuctend = omp_get_wtime();

#if VERBOSITY >= 2
    cout << "[GPU] ~ Table construct time: "
         << tableconstuctend - tableconstuctstart << endl;
#endif
...
#if VERBOSITY >= 2
    cout << "[GPU] ~ Running total of total size of result array after batch "
         << i << ": " << totalResultsLoop << endl;
#endif
  } // pragma omp / for numBatches
  double tEndCompute = omp_get_wtime();
  cout << "[GPU | RESULT] ~ Compute time for the GPU: "
...
void constructNeighborTableKeyValueWithPtrs(
    int *pointIDKey, int *pointInDistValue,
    struct neighborTableLookup *neighborTable, int *pointersToNeighbors,
    unsigned int *cnt) {
  // copy the value data:
