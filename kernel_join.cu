...
__global__ void convertAndResizeDataset(INPUT_DATA_TYPE *in, COMPUTE_TYPE *out,
  ...
  if (tid < nbQueries) {
    ...
    // Fill with 0s so the dimensionality of the dataset is compatible with the
    // tensor cores
    for (int i = INPUT_DATA_DIM; i < COMPUTE_DIM; ++i) {
      out[tid * COMPUTE_DIM + i] = (COMPUTE_TYPE)0.0;
    }
  }
  // The original dataset does not have the 15 supplemental points, so need to
  // do it in another step
  if (tid < ADDITIONAL_POINTS) {
    // Create "fake points" with 0s coordinates so the last query point will
    // still have 15 points after when loading using load_matrix_sync
...
__global__ void preComputedSquaredCoordinates(COMPUTE_TYPE *dataset,
  ...
  for (unsigned int i = 0; i < COMPUTE_DIM; i += 4) {
    ...
    double b =
        dataset[tid * COMPUTE_DIM + i + 1] * dataset[tid * COMPUTE_DIM + i + 1];
    double c =
        dataset[tid * COMPUTE_DIM + i + 2] * dataset[tid * COMPUTE_DIM + i + 2];
    double d =
        dataset[tid * COMPUTE_DIM + i + 3] * dataset[tid * COMPUTE_DIM + i + 3];
...
#pragma unroll
    for (unsigned int j = 0; j < 16; ++j) {
      accum += (ACCUM_TYPE)(dataset[tid * COMPUTE_DIM + i + j]) *
               (ACCUM_TYPE)(dataset[tid * COMPUTE_DIM + i + j]);
    }
...
__global__ void batchEstimatorKernel(
  ...
  for (loopRng[0] = nDMinCellIDs[0]; loopRng[0] <= nDMaxCellIDs[0];
       loopRng[0]++)
    for (loopRng[1] = nDMinCellIDs[1]; loopRng[1] <= nDMaxCellIDs[1];
         loopRng[1]++)
#include "kernelloops.h"
    { // beginning of loop body
      ...
      uint64_t calcLinearID =
          getLinearID_nDimensionsGPUKernelAlt(indexes, nCells, INDEXED_DIM);
      // compare the linear ID with the gridCellLookupArr to determine if the
      // cell is non-empty: this can happen because one point says a cell in a
      // particular dimension is non-empty, but that's because it was related to
      // a different point (not adjacent to the query point)

      struct gridCellLookup tmp;
      tmp.gridLinearID = calcLinearID;

      if (thrust::binary_search(thrust::seq, gridCellLookupArr,
                                gridCellLookupArr + (*nNonEmptyCells),
                                gridCellLookup(tmp))) {
        struct gridCellLookup *resultBinSearch = thrust::lower_bound(
            thrust::seq, gridCellLookupArr,
            gridCellLookupArr + (*nNonEmptyCells), gridCellLookup(tmp));
        unsigned int GridIndex = resultBinSearch->idx;

        for (int k = grid[GridIndex].indexmin; k <= grid[GridIndex].indexmax;
             ++k) {
          ...
          for (int l = 0; l < INPUT_DATA_DIM; ++l) {
            runningTotalDist +=
                (ACCUM_TYPE)((database[dataIdx * COMPUTE_DIM + l] - point[l]) *
        ...
        localCandidateCounter +=
            grid[GridIndex].indexmax - grid[GridIndex].indexmin + 1;
      }
    } // end loop body
...
__device__ void evaluateCell(unsigned int *nCells, unsigned int *indexes,
  ...
  // Compare the linear ID with the gridCellLookupArr to determine if the cell
  // is non-empty: this can happen because one point says a cell in a particular
  // dimension is non-empty, but that's because it was related to a different
  // point (not adjacent to the query point)
  uint64_t calcLinearID =
      getLinearID_nDimensionsGPUKernelAlt(indexes, nCells, INDEXED_DIM);
  ...
  if (thrust::binary_search(thrust::seq, gridCellLookupArr,
    ...
    struct gridCellLookup *resultBinSearch = thrust::lower_bound(
        thrust::seq, gridCellLookupArr, gridCellLookupArr + (*nNonEmptyCells),
        gridCellLookup(tmp));
    unsigned int GridIndex = resultBinSearch->idx;

    // unsigned int nbCandidates = grid[GridIndex].indexmax -
    // grid[GridIndex].indexmin + 1; atomicAdd(cnt, nbCandidates);

    for (int k = grid[GridIndex].indexmin; k <= grid[GridIndex].indexmax; ++k) {
#if ILP == 1
      evalPoint(gridLookupArr, k, database, epsilon, point, cnt, pointIDKey,
                pointInDistVal, pointIdx);
#else
#if KAHAN_CUDA
      evalPointILPKahan(gridLookupArr, k, database, epsilon, point, cnt,
                        pointIDKey, pointInDistVal, pointIdx);
#else
      evalPointILP(gridLookupArr, k, database, epsilon, point, cnt, pointIDKey,
                   pointInDistVal, pointIdx);
#endif
#endif
    }
...
evalPoint(unsigned int *gridLookupArr, int k, COMPUTE_TYPE *database,
  ...
  for (unsigned int i = 0; i < INPUT_DATA_DIM; ++i) {
    runningTotalDist +=
        (ACCUM_TYPE)((database[dataIdx * COMPUTE_DIM + i] - point[i]) *
                     (database[dataIdx * COMPUTE_DIM + i] - point[i]));
...
#pragma unroll
    for (int j = 0; j < ILP && (i + j) < INPUT_DATA_DIM; ++j) {
      runningTotalDist[j] +=
          (ACCUM_TYPE)((database[dataIdx * COMPUTE_DIM + i + j] -
                        point[i + j]) *
                       (database[dataIdx * COMPUTE_DIM + i + j] -
                        point[i + j]));
    }
...
__global__ void distanceCalculationGridCuda(
  ...
  // Calculate the coords of the Cell for the point and the min/max ranges in
  // each dimension
  unsigned int nDCellIDs[INDEXED_DIM];
  unsigned int nDMinCellIDs[INDEXED_DIM];
  unsigned int nDMaxCellIDs[INDEXED_DIM];
  ...
  for (int i = 0; i < INDEXED_DIM; ++i) {
    ...
    nDMinCellIDs[i] = max(
        0, nDCellIDs[i] - 1); // Boundary conditions (don't go beyond cell 0)
    nDMaxCellIDs[i] = min(
        nCells[i] - 1, nDCellIDs[i] + 1); // Boundary conditions (don't go
                                          // beyond the maximum number of cells)
  }
...
#include "kernelloops.h"
    { // beginning of loop body
      ...
      evaluateCell(nCells, indexes, gridCellLookupArr, nNonEmptyCells, database,
                   epsilon, grid, gridLookupArr, point, cnt, pointIDKey,
                   pointInDistVal, originPointIndex[pointId], nDCellIDs);
...
distanceCalculationGridTensor_multiQueryPoints_double_8_8_4_tensor_mixed(
  ...
  if (0 == batchIndex) {
    printf("Thread %d: Tile4 group %d, thread %d, tile8 group %d, thread %d\n",
           warp.thread_rank(), tile4.meta_group_rank(), tile4.thread_rank(),
           tile8.meta_group_rank(), tile8.thread_rank());
  }
#endif

  unsigned int nbStepsToPage = ceil((1.0 * COMPUTE_DIM) / (1.0 * WARP_SIZE));
  unsigned int nbQueriesBatch =
      tensorBatches[batchIndex + 1] - tensorBatches[batchIndex];
    ...
    for (unsigned int i = 0; i < COMPUTE_DIM; i += 4) {
      if ((tile4.thread_rank() + i) < COMPUTE_DIM) {
        sharedArrayQueryPoints[sharedArrayQueryOffset +
                               tile4.meta_group_rank() * COMPUTE_DIM +
                               tile4.thread_rank() + i] =
            database[originPointIndex[tensorBatches[batchIndex] +
  ...
  if (1 == print && batchToPrint == batchIndex && 0 == warp.thread_rank()) {
    ...
    for (unsigned int i = 0; i < 8; ++i) {
      ...
      for (unsigned int j = 0; j < COMPUTE_DIM; ++j) {
        printf("%f, ", sharedArrayQueryPoints[sharedArrayQueryOffset +
                                              i * COMPUTE_DIM + j]);
      }
      printf("\n");
    }
  ...
  // For simplicity, only the 8 first threads of the warp page the squared
  // queries' coordinates into shared memory Should be optimized a bit so each
  // tile4 takes care of a query point instead The 8 squared coordinates of the
  // 8 queries are stored next to each other to improve memory accesses
  // performance E.g.: a0 b0 c0 d0 e0 f0 g0 h0 a1 b1 c1 d1 etc.
  if (warp.thread_rank() < 8) {
    for (unsigned int i = 0; i < (COMPUTE_DIM / 4); ++i) {
      if (warp.thread_rank() < nbQueriesBatch) {
        sharedArraySquaredQueries[sharedArraySquaredOffset + i * 8 +
                                  warp.thread_rank()] =
            preComputedSquaredCoordinates
                [originPointIndex[tensorBatches[batchIndex] +
                                  warp.thread_rank()] *
                     (COMPUTE_DIM / 4) +
                 i];
      } else {
        sharedArraySquaredQueries[sharedArraySquaredOffset + i * 8 +
                                  warp.thread_rank()] = 0.0;
      }
    }
  }
  ...
  if (1 == print && batchToPrint == batchIndex && 0 == warp.thread_rank()) {
    ...
    for (unsigned int i = 0; i < (COMPUTE_DIM / 4); ++i) {
      for (unsigned int j = 0; j < 8; ++j) {
        printf("%f, ",
               sharedArraySquaredQueries[sharedArraySquaredOffset + i * 8 + j]);
      }
      printf("\n");
    }
  }
  ...
  for (unsigned int i = 0; i < INDEXED_DIM; ++i) {
    double queryCoordinateTmp =
        sharedArrayQueryPoints[sharedArrayQueryOffset + i];
...
#include "kernelloops.h"
    { // beginning of loop body
      ...
      if (thrust::binary_search(thrust::seq, gridCellLookupArr,
        ...
        for (unsigned int k = grid[gridIndex].indexmin;
             k <= grid[gridIndex].indexmax; k += 8) {
          ...
          for (unsigned int n = 0; n < COMPUTE_DIM; n += 4) {
#if OUTPUT_DEBUG_TENSOR
            if (1 == print && batchToPrint == batchIndex &&
                0 == warp.thread_rank()) {
              printf("\n\n\n===== n = %d =====\n", n);
            }
#endif
            ...
            if ((k + tile4.meta_group_rank()) < (*nbQueryPoints)) {
              unsigned int candidateId =
                  gridLookupArr[k + tile4.meta_group_rank()];
              sharedArrayTmp8x4[sharedArray8x4Offset +
                                tile4.meta_group_rank() * 4 +
                                tile4.thread_rank()] =
                  database[candidateId * COMPUTE_DIM + n + tile4.thread_rank()];

              if (0 == tile4.thread_rank()) {
                sharedArraySquaredCandidates[warpIdInBlock * 8 +
                                             tile4.meta_group_rank()] =
              ...
              sharedArrayTmp8x4[sharedArray8x4Offset +
                                tile4.meta_group_rank() * 4 +
                                tile4.thread_rank()] = 0.0;
              if (0 == tile4.thread_rank()) {
                sharedArraySquaredCandidates[warpIdInBlock * 8 +
                                             tile4.meta_group_rank()] = 0.0;
              }
            }
            ...
            if (1 == print && batchToPrint == batchIndex &&
              ...
              for (unsigned int i = 0; i < 8; ++i) {
                ...
                for (unsigned int j = 0; j < 4; ++j) {
                  printf("%f, ",
                         sharedArrayTmp8x4[sharedArray8x4Offset + i * 4 + j]);
                }
                printf("\n");
              }

              printf("\nSquared candidate points (C0, C1, C2, C3, C4, C5, C6, "
                     "C7): \n");
              for (unsigned int i = 0; i < 8; ++i) {
                printf("%f, ",
                       sharedArraySquaredCandidates[warpIdInBlock * 8 + i]);
              }
              printf("\n");
            }
#endif
            ...
            wmma::load_matrix_sync(
                matrixQ, sharedArrayQueryPoints + sharedArrayQueryOffset + n,
                COMPUTE_DIM);
            wmma::load_matrix_sync(matrixC,
                                   sharedArrayTmp8x4 + sharedArray8x4Offset, 4);
            wmma::load_matrix_sync(
                matrixC2, sharedArraySquaredCandidates + warpIdInBlock * 8, 0,
                wmma::mem_row_major);
            ...
            wmma::store_matrix_sync(sharedArrayResultTmp + sharedArrayOffset,
                                    matrixQCC2, 8, wmma::mem_row_major);
            ...
            if (1 == print && batchToPrint == batchIndex &&
              ...
              for (unsigned int i = 0; i < 8; ++i) {
                ...
                for (unsigned int j = 0; j < 8; ++j) {
                  printf("%f, ",
                         sharedArrayResultTmp[sharedArrayOffset + i * 8 + j]);
                }
                printf("\n");
              }
            }
            ...
            // Accumulate the previous result with Q^2 for the Euclidean
            // distance of the current dimension Accumulate the previous results
            // with the Euclidean distance from previous dimensions Upper half
            // of the result matrix
            sharedArrayResult[sharedArrayOffset + tile8.meta_group_rank() * 8 +
                              tile8.thread_rank()] =
                sharedArrayResult[sharedArrayOffset +
                                  tile8.meta_group_rank() * 8 +
                                  tile8.thread_rank()] +
                sharedArrayResultTmp[sharedArrayOffset +
                                     tile8.meta_group_rank() * 8 +
                                     tile8.thread_rank()] +
                sharedArraySquaredQueries[sharedArraySquaredOffset +
                                          (n / 4) * 8 +
            ...
            if (1 == print && batchToPrint == batchIndex &&
              ...
              for (unsigned int i = 0; i < 8; ++i) {
                ...
                for (unsigned int j = 0; j < 8; ++j) {
                  printf("%f, ",
                         sharedArrayResult[sharedArrayOffset + i * 8 + j]);
                }
                printf("\n");
              }
            }
            ...
            unsigned int shortCircuitTotal =
                nbQueriesBatch * nbCandidatesCurrent;

            // Upper half of the result matrix
            // Make sure that the threads don't access results that don't exist
            if (tile8.meta_group_rank() < nbQueriesBatch &&
                tile8.thread_rank() < nbCandidatesCurrent) {
              double tmpDistance =
                  fabs(sharedArrayResult[sharedArrayOffset +
                                         tile8.meta_group_rank() * 8 +
                                         tile8.thread_rank()]);
            ...
            if ((tile8.meta_group_rank() + 4) < nbQueriesBatch &&
                tile8.thread_rank() < nbCandidatesCurrent) {
              double tmpDistance =
                  fabs(sharedArrayResult[sharedArrayOffset + 32 +
                                         tile8.meta_group_rank() * 8 +
                                         tile8.thread_rank()]);
              if ((*epsilon) < sqrt(tmpDistance)) {
                shortCircuit++;
            ...
            // Reduction of the number of query/candidate points that should
            // short-circuit their computation
            unsigned int shortCircuitReduce =
                __reduce_add_sync(0xffffffff, shortCircuit);
          ...
          if (tile8.meta_group_rank() < nbQueriesBatch &&
            ...
            if (sqrt(tmpDistance) <= (*epsilon)) {
              ...
              pointIDKey[tmpIdx] = originPointIndex[tensorBatches[batchIndex] +
                                                    tile8.meta_group_rank()];
