...
// #define INPUT_DATA_DIM 384

// Number of dimensions of the data used for the computation on tensor cores
// Multiple of 16 in FP16, multiple of 4 in FP64
// Commented out for makefile
// #define COMPUTE_DIM 384

// Blocksize when using the CUDA cores
...
// Precision of the computation on the GPU
// Tensor cores: 16 or 64
// CUDA cores: 16, 32 or 64
#define COMPUTE_PREC 64

// Precision of the accumulation on the GPU
// Tensor cores: 16, 32, or 64 (cannot combine 16 and 64)
// CUDA cores: 16, 32 or 64
...
// the A and B matrix data, that are (M = 8) * (K = 4) * 8 * (CHUNK_K = 16) *
// sizeof(double) = 32 Kb each But we cannot account the 4 Kb total skew
// overhead, without which the performance would be severely impacted. So we
// choose to reduce the chunk size in half, i.e. the amount of A and B matrix
// data we cache in shared memory. Accordingly, this doubles the number of outer
// iterations across the global K dimension, which only slightly impacts the
// performance.
#if COMPUTE_PREC == 64
#define CHUNK_K 8
#else
...
// thus reducing the number of possible bank conflicts. The number of 4
// eight-byte "double" elements is chosen as the minimum possible shift because
