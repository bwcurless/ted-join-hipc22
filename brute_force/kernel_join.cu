...
__global__ void finishResultMatrix(ACCUM_TYPE *preComputedSquaredCoordinates,
  ...
  for (unsigned int i = 0; i < nbQueryPoints; ++i) {
    ACCUM_TYPE finalDistance = fabs(resultMatrix[i * nbQueryPoints + tid] +
                                    preComputedSquaredCoordinates[i]);
...
// CUDA cores kernel
// Uses 1 thread to compute the distance between 1 query point and all the
// dataset points Uses the standard/textbook Euclidean distance formula
__global__ void distanceCalculationBruteForceCuda(unsigned int *nbQueryPoints,
  ...
  // Uses 1 thread per point, so if the thread id is greater than the number of
  // query points, we return
  if ((*nbQueryPoints) <= tid) {
    return;
  }
...
// CUDA cores kernel
// Uses 1 thread to compute the distance between 1 query point and all the
// dataset points Uses the extended Euclidean distance formula
__global__ void
distanceCalculationBruteForceCudaAlt(unsigned int *nbQueryPoints,
  ...
  for (unsigned int i = 0; i < (*nbQueryPoints); ++i) {
    ...
    for (unsigned int j = 0; j < INPUT_DATA_DIM; ++j) {
      ...
      accumDistance += (ACCUM_TYPE)((COMPUTE_TYPE)(-2.0) * point[j] *
                                    dataset[i * COMPUTE_DIM + j]) +
                       q2[j] + c2;
    }
...
// Tensor cores kernel
// Uses 1 warp to compute the distance between 1 query point and all the dataset
// points Uses the standard/textbook Euclidean distance formula
__global__ void distanceCalculationBruteForceTensorBasic(
  ...
  thread_block_tile<WARP_SIZE> warp =
      tiled_partition<WARP_SIZE>(this_thread_block());
  ...
  wmma::fragment<wmma::accumulator, 16, 16, 16, ACCUM_TYPE>
  ...
  for (unsigned int j = 0; j < COMPUTE_DIM; j += WARP_SIZE) {
    if ((j + warp.thread_rank()) < COMPUTE_DIM) {
      sharedArrayQueryPoints[warpIdInBlock * COMPUTE_DIM + j +
                             warp.thread_rank()] =
  ...
  for (unsigned int i = 0; i < (*nbQueryPoints); i += 16) {
    ...
    for (unsigned int n = 0; n < COMPUTE_DIM; n += 16) {
      ...
      wmma::load_matrix_sync(firstStepAccumulator,
                             dataset + i * COMPUTE_DIM + n, COMPUTE_DIM,
                             wmma::mem_row_major);
      ...
      wmma::store_matrix_sync(sharedArrayResultFirstStep +
                                  sharedArrayResultOffset,
                              firstStepAccumulator, 16, wmma::mem_row_major);

      wmma::load_matrix_sync(
          matrixAFragment, sharedArrayResultFirstStep + sharedArrayResultOffset,
          16);
      wmma::load_matrix_sync(
          matrixBFragment, sharedArrayResultFirstStep + sharedArrayResultOffset,
          16);
    ...
    wmma::store_matrix_sync(sharedArrayResultSecondStep +
                                sharedArrayResultOffset,
    ...
    if (warp.thread_rank() < 16 && warp.thread_rank() < nbCandidatesLeft) {
      ACCUM_TYPE resultDistance =
          sharedArrayResultSecondStep[sharedArrayResultOffset +
                                      warp.thread_rank() * 16 +
                                      warp.thread_rank()];
...
// Tensor cores kernel
// Uses 1 warp to compute the distance between 16 query point and all the
// dataset points Uses the extended Euclidean distance formula
// TODO: Copy and paste this kernel, and make the necessary change to use
// 32x8x16 and 8x32x16 matrix multiplication instead of 16x16x16
// TODO: Overall, for every '16' you see in this code, ask yourself if it should
// be replaced by '8' or '32'
__global__ void distanceCalculationBruteForceTensorHalfOpti(
  ...
  __shared__ ACCUM_TYPE
      sharedArraySquaredQueries[WARP_PER_BLOCK * 16 * (COMPUTE_DIM / 16)];
  // Squared coordinates for the candidate points being computed (up to 16 at a
  // time)
  ...
  // Final result array to accumulate/store the Euclidean distance between the
  // query points and candidate points
  ...
  unsigned int sharedArraySquaredOffset =
      warpIdInBlock * 16 * (COMPUTE_DIM / 16);
  ...
  // If the warp is assigned fewer than 16 query points (e.g., the very last
  // warp), fill the remaining slots with 0
  ...
  if (warp.thread_rank() < 16) {
    for (unsigned int i = 0; i < (COMPUTE_DIM / 16); ++i) {
      if (warp.thread_rank() < nbQueriesBatch) {
        sharedArraySquaredQueries[sharedArraySquaredOffset + i * 16 +
                                  warp.thread_rank()] =
  ...
  for (unsigned int i = 0; i < (*nbQueryPoints); i += 16) {
    ...
    for (unsigned int n = 0; n < COMPUTE_DIM; n += 16) {
      ...
      if (warp.thread_rank() < 16) {
        if ((i + warp.thread_rank()) < (*nbQueryPoints)) {
          ...
          sharedArraySquaredCandidates[warpIdInBlock * 16 +
                                       warp.thread_rank()] =
              preComputedSquaredCoordinates[candidateId * (COMPUTE_DIM / 16) +
                                            (n / 16)];
        } else {
          sharedArraySquaredCandidates[warpIdInBlock * 16 +
                                       warp.thread_rank()] = (ACCUM_TYPE)0.0;
      ...
      // Fragments (i.e., matrices) for the MMA operations using the tensor
      // cores
      ...
      // Candidate points can be loaded from global memory since the accesses
      // are coalesced
      wmma::load_matrix_sync(
          matrixQ, sharedArrayQueryPoints + sharedArrayQueryOffset + n,
          COMPUTE_DIM);
      wmma::load_matrix_sync(matrixC, dataset + i * COMPUTE_DIM + n,
                             COMPUTE_DIM);
      wmma::load_matrix_sync(
          matrixC2, sharedArraySquaredCandidates + (warpIdInBlock * 16), 0,
          wmma::mem_row_major);
      wmma::fill_fragment(matrixQCC2, 0.0);

      // Perform the MMA operation (Q x C + C2, where Q is the query points, C
      // the candidate points, and C2 the squared candidate points
      ...
      wmma::store_matrix_sync(sharedArrayResultTmp + sharedArrayOffset,
                              matrixQCC2, 16, wmma::mem_row_major);

      // Finish computing the Euclidean distance using CUDA cores
      for (unsigned int j = 0; j < 16; j += 2) {
        // Accumulate the previous result (from tensor cores), the Euclidean
        // distance from previous dimensions, and the squared coordinates of the
        // query points (stored in shared memory)
        unsigned int localId = sharedArrayOffset +
                               (j + tile16.meta_group_rank()) * 16 +
                               tile16.thread_rank();
    ...
    // The Euclidean distance between the query points and the candidate points
    // is now computed Check for each pair if the distance is within epsilon or
    // not
    for (unsigned int j = 0; j < 16; j += 2) {
      if ((j + tile16.meta_group_rank()) < nbQueriesBatch &&
          tile16.thread_rank() < nbCandidatesCurrent) {
        ACCUM_TYPE tmpDistance =
            abs(sharedArrayResult[sharedArrayOffset +
                                  (j + tile16.meta_group_rank()) * 16 +
                                  tile16.thread_rank()]);
#if ACCUM_PREC == 16
...
__global__ void distanceCalculationBruteForceTensorDoubleOpti(
  ...
  for (unsigned int i = 0; i < (*nbQueryPoints); i += 8) {
    ...
    for (unsigned int n = 0; n < COMPUTE_DIM; n += 4) {
      ...
      // {
      //     sharedArrayTmp8x4[sharedArray8x4Offset + tile4.meta_group_rank() *
      //     4 + tile4.thread_rank()] =
      ...
      //                 preComputedSquaredCoordinates[(i +
      //                 tile4.meta_group_rank()) * (COMPUTE_DIM / 4) + (n /
      //                 4)];
      //     }
      // } else {
      //     sharedArrayTmp8x4[sharedArray8x4Offset + tile4.meta_group_rank() *
      //     4 + tile4.thread_rank()] = 0.0; if (0 == tile4.thread_rank())
      //     {
      //         sharedArraySquaredCandidates[warpIdInBlock * 8 +
      ...
      if (warp.thread_rank() < 8) {
        if (warp.thread_rank() < nbCandidatesCurrent) {
          sharedArraySquaredCandidates[warpIdInBlock * 8 + warp.thread_rank()] =
              preComputedSquaredCoordinates[(i + warp.thread_rank()) *
                                                (COMPUTE_DIM / 4) +
                                            (n / 4)];
        } else {
          sharedArraySquaredCandidates[warpIdInBlock * 8 + warp.thread_rank()] =
      ...
      //     C7): \n"); for (unsigned int k = 0; k < 8; ++k)
      //     {
      //         printf("%f, ", sharedArraySquaredCandidates[warpIdInBlock * 8 +
      //         k]);
      //     }
      //     printf("\n");
      // }
      ...
      wmma::load_matrix_sync(
          matrixQ, sharedArrayQueryPoints + sharedArrayQueryOffset + n,
          COMPUTE_DIM);
      // wmma::load_matrix_sync(matrixC, sharedArrayTmp8x4 +
      // sharedArray8x4Offset, 4);
      wmma::load_matrix_sync(matrixC, dataset + i * COMPUTE_DIM + n,
                             COMPUTE_DIM);
      wmma::load_matrix_sync(matrixC2,
                             sharedArraySquaredCandidates + warpIdInBlock * 8,
                             0, wmma::mem_row_major);
      wmma::fill_fragment(matrixQCC2, 0.0);
      ...
      //         {
      //             printf("%f, ", sharedArrayResultTmp[sharedArrayOffset + k *
      //             8 + l]);
      //         }
      //         printf("\n");
      //     }
      ...
      //         for (unsigned int l = 0; l < 8; ++l)
      //         {
      //             printf("%f, ", sharedArrayResult[sharedArrayOffset + k * 8
      //             + l]);
      //         }
      //         printf("\n");
      //     }
    ...
    if ((tile8.meta_group_rank() + 4) < nbQueriesBatch &&
        tile8.thread_rank() < nbCandidatesCurrent) {
      double tmpDistance = fabs(
          sharedArrayResult[sharedArrayOffset + 32 +
                            tile8.meta_group_rank() * 8 + tile8.thread_rank()]);
      if (sqrt(tmpDistance) <= (*epsilon)) {
        unsigned int tmpIdx = atomicAdd(cnt, int(1));
      }
    }
