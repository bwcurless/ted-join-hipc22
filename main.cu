...
int main(int argc, char *argv[]) {
  ...
  if (NB_ARGS_MAX != argc) {
    ...
    fprintf(stderr, "[Main] ~   Epsilon: Should be between 0 (excluded) and 1 "
                    "(included).\n");
    ...
    fprintf(
        stderr,
        "[Main] ~      /!\\ WIP /!\\ 12: CUDA cores only, half2 precision.\n");
    fprintf(stderr, "[Main] ~     --- Tensor cores ---\n");
    fprintf(stderr, "[Main] ~     23: CUDA-Tensor cores mix, multiple query "
                    "points per complete computation step.\n");
    return 1;
  }
  ...
  fprintf(
      stdout,
      "[Main | Time] ~ Time to reorder the features by their variance: %f\n",
  ...
  generateNDGridDimensions(&inputVector, epsilon, minArr, maxArr, nCells,
                           &totalCells);
  ...
  gridIndexingGPU(&nbQueryPoints, totalCells, database, &dev_database, &epsilon,
                  &dev_epsilon, minArr, &dev_minArr, &index, &dev_index,
                  indexLookupArr, &dev_indexLookupArr, &gridCellLookupArr,
                  &dev_gridCellLookupArr, &nNonEmptyCells, &dev_nNonEmptyCells,
                  nCells, &dev_nCells);
  ...
  sortByWorkLoad(searchMode, &nbQueryPoints, &totalCandidates,
                 &sortedDatabaseTmp, &epsilon, &dev_epsilon, database,
                 &dev_database, index, &dev_index, indexLookupArr,
                 &dev_indexLookupArr, gridCellLookupArr, &dev_gridCellLookupArr,
                 minArr, &dev_minArr, nCells, &dev_nCells, &nNonEmptyCells,
                 &dev_nNonEmptyCells, &originPointIndex, &dev_originPointIndex);
  ...
  GPUJoinMainIndex(searchMode, database, dev_database, &nbQueryPoints, &epsilon,
                   dev_epsilon, index, dev_index, indexLookupArr,
                   dev_indexLookupArr, gridCellLookupArr, dev_gridCellLookupArr,
                   minArr, dev_minArr, nCells, dev_nCells, &nNonEmptyCells,
                   dev_nNonEmptyCells, originPointIndex, dev_originPointIndex,
                   neighborTable, &pointersToNeighbors, &totalNeighbors,
                   &totalNeighborsCuda, &totalNeighborsTensor,
                   &totalCandidatesCuda, &totalCandidatesTensor,
                   &totalQueriesCuda, &totalQueriesTensor, &totalKernelCuda,
                   &totalKernelTensor, sortedDatabaseTmp);
  double tEndJoin = omp_get_wtime();
  fprintf(stdout, "[Main | Time] ~ Time to join: %f\n", tEndJoin - tStartJoin);

/***** Output the neighbor table (for comparison or checking purposes, or just
 * to keep the result somewhere) *****/
#if OUTPUT_NEIGHBORS
  std::ofstream outputNeighborsFile("output_neighbors.txt");
  for (unsigned int i = 0; i < nbQueryPoints; ++i) {
    std::sort(neighborTable[i].dataPtr + neighborTable[i].indexmin,
              neighborTable[i].dataPtr + neighborTable[i].indexmax + 1);
    outputNeighborsFile << i << ": ";
    for (unsigned int j = neighborTable[i].indexmin;
         j <= neighborTable[i].indexmax; ++j) {
  ...
  unsigned int selectivity =
      abs((long long)(totalNeighbors - nbQueryPoints)) / nbQueryPoints;
  ...
  fprintf(stdout,
          "   [Main | Result] ~ Total result set size CUDA cores: %lu\n",
          totalNeighborsCuda);
  fprintf(stdout,
          "   [Main | Result] ~ Total result set size Tensor cores: %lu\n",
          totalNeighborsTensor);
  fprintf(stdout, "[Main | Result] ~ Query points:\n");
  fprintf(stdout,
          "   [Main | Result] ~ Number of queries computed by CUDA cores: %d\n",
          totalQueriesCuda);
  fprintf(
      stdout,
      "   [Main | Result] ~ Number of queries computed by Tensor cores: %d\n",
      totalQueriesTensor);
  fprintf(stdout, "[Main | Result] ~ Kernel invocations:\n");
  fprintf(stdout,
          "   [Main | Result] ~ Number of kernel invocations using CUDA cores: "
          "%d\n",
          totalKernelCuda);
  fprintf(stdout,
          "   [Main | Result] ~ Number of kernel invocations using Tensor "
          "cores: %d\n",
          totalKernelTensor);
  std::cout << "[Main | Result] ~ Refined candidate points:\n";
  std::cout << "   [Main | Result] ~ Estimated number of candidate points "
               "refined by CUDA cores: "
            << totalCandidatesCuda << '\n';
  std::cout << "   [Main | Result] ~ Estimated number of candidate points "
               "refined by Tensor cores: "
            << totalCandidatesTensor << '\n';
...
/***** Writing the results into file *****/
  ...
  if (inputResultFile.peek() == std::ifstream::traits_type::eof()) {
    ...
    outputResultFile
        << "Dataset, epsilon, searchMode, executionTime, totalNeighbors, "
           "selectivity, inputDim, indexedDim, computeDim, "
        << "blockSize, warpPerBlock, gpuStreams, computePrec, accumPrec, ILP, "
           "shortCircuit, reorderDimByVar, Kahan, reorderDataset\n";
  }
  outputResultFile << filename << ", " << (double)epsilon << ", " << searchMode
                   << ", " << timeBench << ", " << totalNeighbors << ", "
                   << selectivity << ", " << INPUT_DATA_DIM << ", "
                   << INDEXED_DIM << ", " << COMPUTE_DIM << ", " << BLOCKSIZE
                   << ", " << WARP_PER_BLOCK << ", " << GPUSTREAMS << ", "
                   << COMPUTE_PREC << ", " << ACCUM_PREC << ", " << ILP << ", "
                   << SHORT_CIRCUIT << ", " << REORDER_DIM_BY_VAR << ", "
                   << KAHAN_CUDA << ", " << REORDER_DATASET << std::endl;
