...
__device__ uint64_t getLinearID_nDimensionsGPUIndex(unsigned int *indexes,
                                                    unsigned int *dimLen,
...
__global__ void kernelIndexComputeNonEmptyCells(
    INPUT_DATA_TYPE *database, unsigned int *nbQueryPoints, ACCUM_TYPE *epsilon,
    INPUT_DATA_TYPE *minArr, unsigned int *nCells, uint64_t *pointCellArr,
    unsigned int *databaseVal, bool enumerate) {
...
#if ACCUM_PREC == 16
    tmpNDCellIdx[i] =
        ((database[pointID + i] - minArr[i]) / (INPUT_DATA_TYPE)(*epsilon));
#else
  ...
  uint64_t linearID =
      getLinearID_nDimensionsGPUIndex(tmpNDCellIdx, nCells, INDEXED_DIM);
...
void reorderDimensionsByVariance(
...
#if VERBOSITY >= 2
  fprintf(stdout,
          "[Index] ~ Calculating variance based on the following fraction of "
          "points: %f\n",
          inv_sample);
#endif
  ...
  for (int i = 0; i < INPUT_DATA_DIM; ++i) {
    ...
    for (int j = 0; j < (*inputVector).size(); j += sample) {
      sums[i] += (((*inputVector)[j][i]) - average[i]) *
                 (((*inputVector)[j][i]) - average[i]);
    }
  ...
  std::sort(dim_variance, dim_variance + INPUT_DATA_DIM,
            [](const dim_reorder_sort &a, const dim_reorder_sort &b) {
              return a.variance > b.variance;
            });

#if VERBOSITY >= 2
  for (int i = 0; i < INPUT_DATA_DIM; ++i) {
    std::cout << "[Index] ~ Reordering by: dim:" << dim_variance[i].dim
              << ", variance: " << dim_variance[i].variance << '\n';
  }
#endif

#if VERBOSITY >= 2
  std::cout << "[Index] ~ Dimension with greatest variance: "
            << greatest_variance_dim << '\n';
#endif
...
void generateNDGridDimensions(
    std::vector<std::vector<INPUT_DATA_TYPE>> *inputVector, ACCUM_TYPE epsilon,
    INPUT_DATA_TYPE *minArr, INPUT_DATA_TYPE *maxArr, unsigned int *nCells,
    uint64_t *totalCells) {
  fprintf(stdout,
          "[Index] ~ Number of dimensions data: %d, Number of dimensions "
          "indexed: %d\n",
  ...
  // add buffer around each dim so no weirdness later with putting data into
  // cells
  for (int j = 0; j < INDEXED_DIM; ++j) {
#if ACCUM_PREC == 16
...
void gridIndexingGPU(unsigned int *nbQueryPoints, uint64_t totalCells,
                     INPUT_DATA_TYPE *database, INPUT_DATA_TYPE **dev_database,
                     ACCUM_TYPE *epsilon, ACCUM_TYPE **dev_epsilon,
                     INPUT_DATA_TYPE *minArr, INPUT_DATA_TYPE **dev_minArr,
                     struct grid **index, struct grid **dev_index,
                     unsigned int *indexLookupArr,
                     unsigned int **dev_indexLookupArr,
                     struct gridCellLookup **gridCellLookupArr,
                     struct gridCellLookup **dev_gridCellLookupArr,
                     unsigned int *nNonEmptyCells,
                     unsigned int **dev_nNonEmptyCells, unsigned int *nCells,
                     unsigned int **dev_nCells) {
  cudaError_t errCode;

  cudaErrCheck(cudaMalloc((void **)dev_database,
  ...
  cudaErrCheck(
      cudaMalloc((void **)dev_minArr, sizeof(INPUT_DATA_TYPE) * (INDEXED_DIM)));

  cudaErrCheck(cudaMalloc((void **)dev_indexLookupArr,
                          sizeof(unsigned int) * (*nbQueryPoints)));

  cudaErrCheck(cudaMalloc((void **)dev_nNonEmptyCells, sizeof(unsigned int)));

  cudaErrCheck(
      cudaMalloc((void **)dev_nCells, sizeof(unsigned int) * (INDEXED_DIM)));

  uint64_t *dev_pointCellArr;
  cudaErrCheck(cudaMalloc((void **)&dev_pointCellArr,
                          sizeof(uint64_t) * (*nbQueryPoints)));

  unsigned int *dev_databaseVal;
  cudaErrCheck(cudaMalloc((void **)&dev_databaseVal,
                          sizeof(unsigned int) * (*nbQueryPoints)));
  ...
  cudaErrCheck(cudaMemcpy((*dev_database), database,
                          sizeof(INPUT_DATA_TYPE) * (COMPUTE_DIM) *
                              ((*nbQueryPoints) + ADDITIONAL_POINTS),
  ...
  cudaErrCheck(cudaMemcpy((*dev_minArr), minArr,
                          sizeof(INPUT_DATA_TYPE) * (INDEXED_DIM),
                          cudaMemcpyHostToDevice));

  cudaErrCheck(cudaMemcpy((*dev_nCells), nCells,
                          sizeof(unsigned int) * (INDEXED_DIM),
                          cudaMemcpyHostToDevice));

  cudaErrCheck(cudaMemcpy(dev_N, nbQueryPoints, sizeof(unsigned int),
                          cudaMemcpyHostToDevice));
  ...
  kernelIndexComputeNonEmptyCells<<<TOTALBLOCKS, BLOCKSIZE>>>(
      (*dev_database), dev_N, (*dev_epsilon), (*dev_minArr), (*dev_nCells),
      dev_pointCellArr, nullptr, false);
  ...
  try {
    // first sort
    thrust::sort(thrust::device, dev_pointCellArr_ptr,
                 dev_pointCellArr_ptr +
                     (*nbQueryPoints)); //, thrust::greater<uint64_t>()
    // then unique
    dev_new_end = thrust::unique(thrust::device, dev_pointCellArr_ptr,
                                 dev_pointCellArr_ptr + (*nbQueryPoints));
  ...
  *nNonEmptyCells = numNonEmptyCells;
  ...
  cudaErrCheck(cudaMemcpy(pointCellArrTmp, dev_pointCellArr,
                          sizeof(uint64_t) * numNonEmptyCells,
  ...
  kernelIndexComputeNonEmptyCells<<<TOTALBLOCKS, BLOCKSIZE>>>(
      (*dev_database), dev_N, (*dev_epsilon), (*dev_minArr), (*dev_nCells),
      dev_pointCellArr, dev_databaseVal, true);

  try {
    thrust::sort_by_key(thrust::device, dev_pointCellArr,
                        dev_pointCellArr + (*nbQueryPoints), dev_databaseVal);
  } catch (std::bad_alloc &e) {
    fprintf(stderr,
            "[Index] ~ Ran out of memory while sorting key/value pairs\n");
    exit(1);
  }

  uint64_t *cellKey = new uint64_t[(*nbQueryPoints)];
  cudaErrCheck(cudaMemcpy(cellKey, dev_pointCellArr,
                          sizeof(uint64_t) * (*nbQueryPoints),
                          cudaMemcpyDeviceToHost));

  cudaErrCheck(cudaMemcpy(indexLookupArr, dev_databaseVal,
                          sizeof(unsigned int) * (*nbQueryPoints),
                          cudaMemcpyDeviceToHost));
  ...
  fprintf(stdout, "[Index] ~ Full cells: %d (%f, fraction full)\n",
          ...
          numNonEmptyCells * 1.0 / double(totalCells));
  fprintf(stdout, "[Index] ~ Empty cells: %ld (%f, fraction empty)\n",
          totalCells - (unsigned int)numNonEmptyCells,
          (totalCells - numNonEmptyCells * 1.0) / double(totalCells));
  fprintf(stdout,
          "[Index] ~ Size of index that would be sent to GPU (GiB) -- (if full "
          "index sent), excluding the data lookup arr: %f\n",
          (double)sizeof(struct grid) * (totalCells) /
              (1024.0 * 1024.0 * 1024.0));
  fprintf(stdout,
          "[Index] ~ Size of compressed index to be sent to GPU (GiB), "
          "excluding the data and grid lookup arr: %f\n",
          (double)sizeof(struct grid) * (numNonEmptyCells * 1.0) /
              (1024.0 * 1024.0 * 1024.0));
  fprintf(stdout,
          "[Index] ~ When copying from entire index to compressed index: "
          "number of non-empty cells: %lu\n",
          numNonEmptyCells);
#endif
  ...
  cudaErrCheck(
      cudaMalloc((void **)dev_index, sizeof(struct grid) * (*nNonEmptyCells)));

  cudaErrCheck(cudaMalloc((void **)dev_gridCellLookupArr,
                          sizeof(struct gridCellLookup) * (*nNonEmptyCells)));

  ////////////////////////////////////////////////////////////////////////////

  cudaErrCheck(cudaMemcpy((*dev_nNonEmptyCells), nNonEmptyCells,
                          sizeof(unsigned int), cudaMemcpyHostToDevice));

  cudaErrCheck(cudaMemcpy((*dev_index), (*index),
                          sizeof(struct grid) * numNonEmptyCells,
                          cudaMemcpyHostToDevice));

  cudaErrCheck(cudaMemcpy((*dev_indexLookupArr), indexLookupArr,
                          sizeof(unsigned int) * (*nbQueryPoints),
                          cudaMemcpyHostToDevice));

  cudaErrCheck(cudaMemcpy((*dev_gridCellLookupArr), (*gridCellLookupArr),
                          sizeof(struct gridCellLookup) * numNonEmptyCells,
                          cudaMemcpyHostToDevice));
