...
__global__ void sortByWorkLoadGlobal(
    INPUT_DATA_TYPE *database, ACCUM_TYPE *epsilon, struct grid *index,
    unsigned int *indexLookupArr, struct gridCellLookup *gridCellLookupArr,
    INPUT_DATA_TYPE *minArr, unsigned int *nCells, unsigned int *nNonEmptyCells,
  ...
  // for (loopRng[0] = rangeFilteredCellIdsMin[0]; loopRng[0] <=
  // rangeFilteredCellIdsMax[0]; loopRng[0]++) 	for (loopRng[1] =
  // rangeFilteredCellIdsMin[1]; loopRng[1] <= rangeFilteredCellIdsMax[1];
  // loopRng[1]++)
...
#include "kernelloops.h"
    {
      ...
      uint64_t cellID =
          getLinearID_nDimensionsGPUSortByWL(indexes, nCells, INDEXED_DIM);
      ...
      if (thrust::binary_search(thrust::seq, gridCellLookupArr,
        ...
        nbNeighborPoints +=
            index[GridIndex].indexmax - index[GridIndex].indexmin + 1;
      }
    }
...
void sortByWorkLoad(
    unsigned int searchMode, unsigned int *nbQueryPoints,
    boost::multiprecision::uint128_t *totalCandidates,
    struct schedulingCell **sortedDatabaseTmp, ACCUM_TYPE *epsilon,
    ACCUM_TYPE **dev_epsilon, INPUT_DATA_TYPE *database,
    INPUT_DATA_TYPE **dev_database, struct grid *index, struct grid **dev_index,
    unsigned int *indexLookupArr, unsigned int **dev_indexLookupArr,
    struct gridCellLookup *gridCellLookupArr,
    struct gridCellLookup **dev_gridCellLookupArr, INPUT_DATA_TYPE *minArr,
    INPUT_DATA_TYPE **dev_minArr, unsigned int *nCells,
    unsigned int **dev_nCells, unsigned int *nNonEmptyCells,
    unsigned int **dev_nNonEmptyCells, unsigned int **originPointIndex,
  ...
  // struct schedulingCell * sortedDatabaseTmp = new
  // schedulingCell[sizeof(struct schedulingCell) * (*nNonEmptyCells)];
  (*sortedDatabaseTmp) =
      new schedulingCell[sizeof(struct schedulingCell) * (*nNonEmptyCells)];

  struct schedulingCell *dev_sortedDatabaseTmp;
  cudaErrCheck(cudaMalloc((void **)&dev_sortedDatabaseTmp,
                          sizeof(struct schedulingCell) * (*nNonEmptyCells)));

  // Beginning of the sorting section
  int nbBlock = ((*nNonEmptyCells) / BLOCKSIZE) + 1;

  cudaEventRecord(startKernel);
  sortByWorkLoadGlobal<<<nbBlock, BLOCKSIZE>>>(
      (*dev_database), (*dev_epsilon), (*dev_index), (*dev_indexLookupArr),
      (*dev_gridCellLookupArr), (*dev_minArr), (*dev_nCells),
      (*dev_nNonEmptyCells), dev_sortedDatabaseTmp);
  cudaEventRecord(endKernel);

  cudaDeviceSynchronize();

  cudaErrCheck(cudaMemcpy((*sortedDatabaseTmp), dev_sortedDatabaseTmp,
                          sizeof(struct schedulingCell) * (*nNonEmptyCells),
  ...
  std::sort((*sortedDatabaseTmp), (*sortedDatabaseTmp) + (*nNonEmptyCells),
            [](const schedulingCell &a, const schedulingCell &b) {
  ...
  unsigned int minNeighbor =
      (*sortedDatabaseTmp)[(*nNonEmptyCells) - 1].nbPoints;
...
#if REORDER_DATASET
  (*reorderedDataset) =
      new INPUT_DATA_TYPE[(*nbQueryPoints) + ADDITIONAL_POINTS];
#endif
  ...
  for (int i = 0; i < (*nNonEmptyCells); ++i) {
    ...
    accNeighbor += (((unsigned long long)nbNeighbor) *
                    (unsigned long long)(*sortedDatabaseTmp)[i].nbPoints);
  ...
  cudaErrCheck(cudaMalloc((void **)dev_originPointIndex,
                          (*nbQueryPoints) * sizeof(unsigned int)));

  cudaErrCheck(cudaMemcpy((*dev_originPointIndex), (*originPointIndex),
                          (*nbQueryPoints) * sizeof(unsigned int),
  ...
  std::cout << "[Sort | Result] ~ Total number of candidate points to refine: "
            << accNeighbor << '\n';
  fprintf(stdout,
          "[Sort | Result] ~ Number of candidates: min = %d, median = %d, max "
          "= %d\n",
          minNeighbor, (*sortedDatabaseTmp)[(*nNonEmptyCells) / 2].nbPoints,
          maxNeighbor);
  fprintf(stdout, "[Sort] ~ Deciles number of candidates:\n");
  for (int i = 1; i < 10; ++i) {
    fprintf(stdout, "   [Sort] ~ %d = %d\n", i * 10,
            (*sortedDatabaseTmp)[decileMark * i].nbPoints);
  }
