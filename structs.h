...
// and the size of each of these arrays (needed to construct a subsequent
// neighbor table) will be used inside a vector.
...
// index lookup table for the GPU. Contains the indices for each point in an
// array where the array stores the direct neighbours of all of the points
...
struct grid {
  int indexmin; // Contains the indices for each point in an array where the
                // array stores the ids of the points in the grid
  int indexmax;
};

// key/value pair for the gridCellLookup -- maps the location in an array of
// non-empty cells
